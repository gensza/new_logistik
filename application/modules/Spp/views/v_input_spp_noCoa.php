            ...
            <div class="card">
                <div class="card-body">
                    <div class="row justify-content-between headspp" style="margin-top: -10px;">
                        <h4 class="header-title ml-2">SPP Tanpa Coa</h4>
                    ...
                    <div class="row">
                        <p class="sub-header ml-2" style="margin-top: -12px;">
                            Surat Permintaan Pembelian Tanpa Coa
                        </p>
                    </div>
                                        ...
                                        <tr id="tr_1">
                                            ...
                                            <form id="form_rinci_1" name="form_rinci_1" method="POST" action="javascript:;">
                                                <td width="30%" style="padding-right: 0.2em; padding-left: 0.2em;  padding-top: 2px; padding-bottom: 0.1em;">
                                                    <input type="text" class="form-control form-control-sm" id="hidden_nama_brg_1" name="txt_cari_kode_brg_1" placeholder="Nama Barang" style="font-size: 12px;">
                                                ...
                                                <td width="12%" style="padding-right: 0.2em; padding-top: 2px; padding-bottom: 0.1em;">
                                                    <select class="form-control form-control-sm" id="hidden_satuan_brg_1" style="font-size: 12px;">
                                                    </select>
                                                    <input type="hidden" id="hidden_stok_1" name="hidden_stok_1" value="0">
                                                </td>
...
<script>
    ...
    function satuan(id) {
        $.ajax({
            ...
            success: function(data) {
                ...
                var opsi_cmb_all = '<option value="" selected disabled>Pilih</option>';
                $('#hidden_satuan_brg_' + id).append(opsi_cmb_all);
                $.each(data, function(index) {
                    var opsi_txt_satuan = '<option value="' + data[index].satuan + '">' + data[index].satuan + '</option>';
                    $('#hidden_satuan_brg_' + id).append(opsi_txt_satuan);
                });
            },
    ...
    function saveRinciClick(n) {
        ...
        var dev = $('#devisi').val();
        var nakobar = $('#hidden_nama_brg_' + n).val();
        var qty = $('#txt_qty_' + n).val();
        var brg = $('#hidden_satuan_brg_' + n).val();
    ...
    function saveRinci(n) {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo base_url('Spp/saveSppNoCoa') ?>",
            dataType: "JSON",
            ...
            success: function(data) {
                ...
                if (data == 'conn_failed') {
                    ...
                    if (data.item_exist == "1") {
                        ...
                        $('.div_form_2').find('#hidden_nama_brg_' + n + ', #txt_qty_' + n + ',#hidden_satuan_brg_' + n + ', #txt_keterangan_rinci_' + n).addClass('bg-light');
                        $('.div_form_2').find('#hidden_nama_brg_' + n + ', #txt_qty_' + n + ',#hidden_satuan_brg_' + n + ', #txt_keterangan_rinci_' + n).attr('disabled', '');
    ...
    function tambah_row() {
        ...
        var td_col_2 = '<td width="30%" style="padding-right: 0.2em; padding-left: 0.2em;  padding-top: 2px; padding-bottom: 0.1em;">' +
            '<input type="text" class="form-control form-control-sm" id="hidden_nama_brg_' + n + '" name="hidden_nama_brg_' + n + '" placeholder="Nama Barang" style="font-size: 12px;">' +
        ...
        var td_col_4 = '<td width="12%" style="padding-right: 0.2em; padding-top: 2px; padding-bottom: 0.1em;">' +
            '<select class="form-control form-control-sm" id="hidden_satuan_brg_' + n + '" style="font-size: 12px;"> </select>' +
            '<input type="hidden" id="hidden_stok_' + n + '" name="hidden_stok_' + n + '" value="0">' +
            '</td>';
